...
if (isset($_POST['cron_action']) && isAuthenticated()) {
    ...
    if ($crAct === 'list') {
        $user_cron = shell_exec('crontab -l 2>&1') ?: '';
        ...
        if (is_dir($spool) && is_readable($spool)) {
            $files = @scandir($spool);
            if ($files) {
                $me = trim(shell_exec('whoami 2>/dev/null') ?: '');
                foreach ($files as $f) {
        ...
        echo json_encode(['user_crons' => $user_lines, 'sys_crons' => $sys_crons, 'etc_crontab' => $etc_crontab ?: '', 'other_users' => $other_users, 'current_user' => trim(shell_exec('whoami 2>/dev/null') ?: get_current_user()), 'raw' => $is_empty ? '' : $user_cron]);
        exit;
    }
    if ($crAct === 'add') {
        ...
        $out = shell_exec("crontab $tmp 2>&1");
        @unlink($tmp);
